...
import { Box, Avatar, Typography, Card, CardContent, CircularProgress, Drawer, Button, Chip } from '@mui/material';
...
const FriendList = () => {
    ...
    useEffect(() => {
        const fetchFriends = async () => {
            setLoading(true);
            try {
                const res = await instance.get(`/get-user-friends/${state.userData.user_id}`);
                setFriends(res.data);
            } catch (err) {
                setFriends([]);
            } finally {
        ...
        if (state.userData?.user_id) fetchFriends();
    }, [state.userData]);
    ...
    const handleAvatarClick = async (friend) => {
        ...
        try {
            const res = await instance.get(`/get-profile/${friend.user_id}`);
            setSelectedProfile(res.data.user);
            // Fetch all images for the user
            const imgRes = await instance.get(`/user-image/get-user-images/${friend.user_id}`);
    ...
    return (
        <Box sx={{ p: 3 }}>
            <Typography variant="h5" sx={{ mb: 2, fontWeight: 'bold' }}>Your Friends</Typography>
            {friends.length === 0 && <Typography>No friends found.</Typography>}
            {friends.map(friend => (
                <Card key={friend.user_id} sx={{ mb: 2, display: 'flex', alignItems: 'center', p: 2 }}>
                    <Avatar src={friend.avatar || ''} sx={{ mr: 2, cursor: 'pointer' }} onClick={() => handleAvatarClick(friend)} />
